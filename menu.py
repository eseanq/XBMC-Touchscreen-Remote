...
class cMenu:
   ...
   def __init__(self, x, y, h_pad, v_pad, orientation, number, background,
      ...
      self.refresh_whole_surface_on_load = True# When the menu is first
...
#### El_P functions ###################################
# Checks if mouse click on menu item

   def findItem(self, mpos):
       a = None
       for button in self.menu_items:
           # This line creates a Rect item of the button's absolute position
           # This may be better created once when the button is created rather than
           # calculating on the fly!
           but_loc = pygame.Rect(button['offset'][0],button['offset'][1], button['rect'][2], button['rect'][3])
           if but_loc.collidepoint(mpos):
               # Mouse clicked on a button!
               
               # Get the desired state of the button
               a = button['state']
               
               # Click the button and perform any attached action
   ...
   def button_click(self, button):
       ...
       # Check whether an action was attached to the button
       if not button['action'] is None:
          # if it is, execute it! 
          button['action']()
          
   def nextPage(self, page=0):
       if page > 0:
           self.page += page
       else:
           self.page += 1
       
   def prevPage(self, page=0):
       if page > 0:
           self.page -= page
       else:
           self.page -= 1
   ...
   def setTVShow(self, tvshowid):
       self.page = 0
       self.tvshow = tvshowid
       
   def setTVSeason(self, tvseason):
       self.page = 0
       self.tvseason = tvseason
       
   def setArtist(self, artistid):
   ...
   def update(self, e, c_state):
      ...
      if e.type == pygame.MOUSEBUTTONDOWN:
         ...
         selitem = self.findItem(mpos)
         if not selitem is None:
             return [pygame.Rect((0, 0), self.draw_surface.get_size())], selitem
